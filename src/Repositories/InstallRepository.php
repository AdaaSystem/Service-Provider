...
namespace Adaa\Service\Repositories;
...
class InstallRepository
{
    ...
    public function checkReinstall()
    {
        try {
            ...
            return (Storage::exists('.install_count') ? Storage::get('.install_count') : 0) and (Artisan::call('Adaa:migrate-status'));
        } catch (Exception $e) {
            return false;
        }
    }
    ...
    public function getPreRequisite()
    {
        ...
        $server[] = $this->check($this->my_version_compare(phpversion(), config('Adaa.php_version', '7.2.0'), '>='), sprintf('Min PHP version ' . config('Adaa.php_version', '7.2.0') . ' (%s)', 'Current Version ' . phpversion()), 'Current Version ' . phpversion(), true);
        ...
        $verifier = verifyUrl(config('Adaa.verifier', 'auth'));
    ...
    public function validateLicense($params)
    {
        ...
        $url = verifyUrl(config('Adaa.verifier', 'auth')) . '/api/cc?a=install&u=' . app_url() . '&ac=' . request('access_code') . '&i=' . config('app.item') . '&e=' . request('envato_email') . '&ri=' . request('re_install') . '&current=' . urlencode(request()->path()) . '&ve=' . $ve.'&v='.$v;
    ...
    public function checkLicense()
    {
        ...
        $url = verifyUrl(config('Adaa.verifier', 'auth')) . '/api/cc?a=verify&u=' . app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v . '&current=' . urlencode(request()->path());
        $response = curlIt($url);
    ...
    public function migrateDB()
    {
        try {
            ...
            $this->rollbackDb();
            Log::error($e);
            $sql = base_path('database/' . config('Adaa.database_file'));
            if (File::exists($sql)) {
    ...
    public function installModule($params)
    {
        ...
        if (!$row) {
            if (gbv($params, 'file')) {
                ...
                if (!Schema::hasColumn(config('Adaa.settings_table'), $name)) {
                    Schema::table(config('Adaa.settings_table'), function ($table) use ($name) {
                        $table->integer($name)->default(1)->nullable();
                    });
                }
            }
        } else {
            $settings_model_name = config('Adaa.settings_model');
        ...
        if ($status) {
            ...
            try {
                ...
                DB::beginTransaction();
                $module_class_name = config('Adaa.module_manager_model');
    ...
    protected function disableModule($module_name, $row = false, $file = false)
    {
        ...
        $module_model_name = config('Adaa.module_model');
    ...
    public function installTheme($params)
    {
        ...
        $query = DB::table(config('Adaa.theme_table', 'themes'))->where('name', $name);
        $theme = $query->first();
        ...
        $url =  verifyUrl(config('Adaa.verifier', 'auth')) . '/api/cc?a=install&u=' . app_url() . '&ac=' . $code . '&i=' . $item_id . '&e=' . $e . '&t=Theme';
