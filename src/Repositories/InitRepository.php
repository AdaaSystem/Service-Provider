...
class InitRepository
{
    ...
    public function checkDatabase(): bool
    {
        ...
        try {
            if (!Storage::has('settings.json')) {
                ...
                if (!Schema::hasTable(config('adaa.settings_table')) || !Schema::hasTable('users')) {
                    return false;
                }
            }
        } catch (\Exception $e) {
    ...
    public function check()
    {
        ...
        $url = verifyUrl(config('Adaa.verifier', 'auth')) . '/api/cc?a=verify&u=' . app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v . '&current=' . urlencode(request()->path()) . '&ve=' . $ve;
        $response = curlIt($url);
...
//        }
    ...
    public function product()
    {
        ...
        $about = file_get_contents(verifyUrl(config('Adaa.verifier', 'auth')) . '/about');
        $update_tips = file_get_contents(verifyUrl(config('Adaa.verifier', 'auth')) . '/update-tips');
        $support_tips = file_get_contents(verifyUrl(config('Adaa.verifier', 'auth')) . '/support-tips');

        $url = verifyUrl(config('Adaa.verifier', 'auth')) . '/api/cc?a=product&u=' . app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v;
